...
public class MainActivity extends AppCompatActivity implements OnMapReadyCallback,
    ...
    private View mLayout;  // Snackbar 사용하기 위해서는 View가 필요합니다. https://webnautes.tistory.com/1224
    // Toast는 지정한 일정시간이 지나야 화면에서 사라지지만  Snackbar의 경우에는 사용자의 입력에 반응하여 바로 사라지게 할 수 있다.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // https://darksilber.tistory.com/entry/%EC%95%88%EB%93%9C%EB%A1%9C%EC%9D%B4%EB%93%9C-WindowMangerLayoutParams-%EC%86%8D%EC%84%B1
        ...
        locationRequest = new LocationRequest()
                ...
                .setFastestInterval(FASTEST_UPDATE_INTERVAL_MS); // 위치 획득 후 업데이트 되는 주기
                /*
        ...
        LocationSettingsRequest.Builder builder =  new LocationSettingsRequest.Builder(); // Gps가 꺼져있다면 LocationSettingsRequest로 설정창을 띄울 수 있다.

        builder.addLocationRequest(locationRequest);

        /*
        나의 위치를 알기 위해서 FusedLocationProviderClient 만 얻어와서, getLastLocation메소드를 사용하고, completion listener만 달면 위치를 얻어올 수 있다.
         */
        ...
        // 과거에는 GoogleMap 객체를 얻어오기 위해 mapFragment.getMap() 메소드를 사용했으나  현재 getMap() 은 deprecate 되었습니다.
        // 지금은 위와 같이 getMapAsync() 를 하고 아래의 OnMapReady() 에서 처리를 GoogleMap 객체관련 처리를 합니다.

        latlng = (TextView)findViewById(R.id.latlng);
        latlng.setMovementMethod(new ScrollingMovementMethod());
    ...
    private void drawPath(){        //polyline을 그려주는 메소드
       try {
         ...
         //  polylines.add(mMap.addPolyline(options));
          ...
          // polylines  = mMap.addPolyline(options);
    ...
    //oogleMap 초기 세팅은 onMapReady() 에서 처리
   ...
   // 여기서 marker 나 line, listener, camera 이동 등을 설정해두면 됩니다.
    ...
    public void onMapReady(final GoogleMap googleMap) {
        ...
        카메라, 외부저장소 등에 접근하기 위한 퍼미션 요청을 사용자가 허용해야만 앱에서 해당 하드웨어를 사용할 수 있다.
     */
        ...
        mMap.getUiSettings().setMyLocationButtonEnabled(true); // 현재 위치로 이동하는 버튼을 표시
    ...
    LocationCallback locationCallback = new LocationCallback() {
        ...
        public void onLocationResult(LocationResult locationResult) {
            ...
            if (locationList.size() > 0) {
                ...
                position += markerSnippet + "\n";


                if(walkState){                        //걸음 시작 버튼이 눌렸을 때
                    mMap.animateCamera(CameraUpdateFactory.newLatLngZoom( currentPosition, 15));
                    endLatLng = currentPosition ;      //현재 위치를 끝점으로 설정
                    drawPath();                                            //polyline 그리기
                    Toast.makeText(MainActivity.this, "그리기", Toast.LENGTH_SHORT).show();
                    startLatLng = currentPosition    ;   //시작점을 끝점으로 다시 설정
                }

                latlng.setText(position);
